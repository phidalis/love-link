...
function processPayment(plan, paymentModal) {
    ...
    setTimeout(() => {
        ...
        if (isSuccess) {
            ...
            const adminSettings = JSON.parse(localStorage.getItem('lovelink_admin_settings') || '{}');
            const price = plan === 'premium' ? (adminSettings.premiumPrice || 19.99) : (adminSettings.premiumPlusPrice || 29.99);
            confirmBtn.textContent = `Pay $${price}`;
        }
    }, 1500);
}
...
async function completePremiumUpgrade(plan) {
    ...
    // Clear lockout from Firestore
    const lockoutData = JSON.parse(localStorage.getItem('lovelink_lockout') || '{}');
    delete lockoutData[userId];
    localStorage.setItem('lovelink_lockout', JSON.stringify(lockoutData));
