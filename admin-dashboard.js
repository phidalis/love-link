...
// ── Auth Guard (localStorage quick-check before Firebase confirms) ────────────
...
// ── Init: wait for Firebase Auth to confirm identity before any Firestore calls
// This prevents "Missing or insufficient permissions" caused by queries firing
// before the auth token is attached to the request.
document.addEventListener('DOMContentLoaded', function () {
    // Poll until Firebase is mounted by the module script in the HTML
    var pollInterval = setInterval(function () {
        if (!window.auth || !window.onAuthStateChanged) return;
        clearInterval(pollInterval);

        window.onAuthStateChanged(window.auth, async function (firebaseUser) {
            if (!firebaseUser) {
                // Firebase session expired — clear local session and redirect
                localStorage.removeItem('lovelink_admin_session');
                window.location.href = 'admin-login.html';
                return;
            }

            // Re-sync session email/role from Firestore in case it changed
            try {
                var adminSnap = await window.fsGetDoc(window.fsDoc(window.db, 'admins', firebaseUser.uid));
                if (adminSnap.exists()) {
                    var data = adminSnap.data();
                    adminSession.role  = data.role  || adminSession.role;
                    adminSession.email = data.email || adminSession.email;
                    adminSession.name  = data.name  || adminSession.name;
                    localStorage.setItem('lovelink_admin_session', JSON.stringify(adminSession));
                }
            } catch(e) { /* non-fatal — use cached session */ }

            document.getElementById('adminEmailDisplay').textContent = adminSession.email || firebaseUser.email || 'admin@lovelink.com';
...
async function confirmSuspend(userId) {
    ...
    showToast((allUsers[idx].name || 'User') + ' has been suspended', 'warning');
...
async function unsuspendUser(userId) {
    ...
    await saveUserToFirestore(allUsers[idx]);
    showToast((allUsers[idx].name || 'User') + ' unsuspended', 'success');
    if (currentPage === 'users') renderUsersTable(); else renderDashboard();
}
...
async function banUser(userId) {
    ...
    await saveUserToFirestore(allUsers[idx]);
    showToast((allUsers[idx].name || 'User') + ' banned', 'error');
    if (currentPage === 'users') renderUsersTable(); else renderDashboard();
}
...
async function renderSettings() {
    ...
    document.getElementById('pageContent').innerHTML =
        '<div class="settings-section">' +
            '<h3><i class="fas fa-coins"></i> Token System Settings</h3>' +
            '<div class="settings-group"><label>Free Tokens per Session</label><input type="number" id="freeTokens" value="' + (s.freeTokens || 25) + '" min="5" max="100"></div>' +
            '<div class="settings-group"><label>Token Reset Hours</label><input type="number" id="tokenResetHours" value="' + (s.tokenResetHours || 6) + '" min="1" max="24"></div>' +
            ...
            '<button class="save-settings-btn" onclick="saveTokenSettings()"><i class="fas fa-save"></i> Save Token Settings</button>' +
        '</div>' +
        '<div class="settings-section">' +
...
async function saveTokenSettings() {
    var s = await getAdminSettings();
    s.freeTokens = parseInt(document.getElementById('freeTokens').value) || 25;
    s.tokenResetHours = parseInt(document.getElementById('tokenResetHours').value) || 6;
    s.freeLikes = parseInt(document.getElementById('freeLikes').value) || 5;
    await window.fsSetDoc(docRef('settings', 'admin'), s, { merge: true });
    showToast('Token settings saved — active immediately for new sessions', 'success');
}
