...
// NOTE: getUsers/saveUsers replaced by Firestore — kept as stubs to avoid errors in any legacy callers
function getUsers() { return []; }
function saveUsers(users) { /* no-op: data lives in Firestore */ }
    ...
    setTimeout(() => {
        ...
        if (form) {
            ...
            newForm.addEventListener('submit', async (e) => {
                ...
                // Save contact message to Firestore (not localStorage)
                try {
                    const msgId = 'msg_' + Date.now() + '_' + Math.random().toString(36).substr(2, 6);
                    await window.setDoc(window.doc(window.db, 'contact_messages', msgId), {
                        id: msgId,
                        name: name,
                        email: email,
                        subject: subject,
