...
let timeRemaining = 0; // kept for legacy compat but unused
let tokensRemaining = 25;
...
async function handleLike(userId, btn) {
    ...
    // Consume 1 token for free users
    if (!isPremiumUser) {
        const ok = await useToken('like');
        if (!ok) return;
    }
...
async function handleAdmire(userId, btn) {
    ...
    // Premium users: consume 1 token only if somehow non-premium slips through (safety)
    // Admire is premium-only so no token deduction needed
    ...
    if (inputField) {
        inputField.addEventListener('keydown', e => {
            if (e.key === 'Enter' && !e.shiftKey) { 
                e.preventDefault(); 
                if (window._readOnlyModeActive === true) {
                    showToast('🔒 Your free tokens are used up. Upgrade to continue messaging!', 'warning');
                    openSubscriptionModal();
                    return;
                }
...
async function sendChatMessageSection() {
    ...
    if (!isPremiumUser) {
        const ok = await useToken('message');
        if (!ok) return;
    }
...
// ─── Token System ─────────────────────────────────────────────────────────────
const FREE_TOKEN_MAX = 25;
const FREE_TOKEN_RESET_HOURS = 6;
...
async function startTimerIfNeeded() {
    ...
    const lastReset = session.lastReset || 0;
    const hoursSinceReset = (now - lastReset) / (1000 * 60 * 60);

    if (!session.tokens && session.tokens !== 0) {
        // First time — initialize tokens
        tokensRemaining = FREE_TOKEN_MAX;
        await updateDoc(doc(db, 'users', currentUser.id), {
            'freeSession.tokens': FREE_TOKEN_MAX,
            'freeSession.lastReset': now
        });
    } else if (hoursSinceReset >= FREE_TOKEN_RESET_HOURS) {
        // Reset tokens after 6 hours
        tokensRemaining = FREE_TOKEN_MAX;
        await updateDoc(doc(db, 'users', currentUser.id), {
            'freeSession.tokens': FREE_TOKEN_MAX,
            'freeSession.lastReset': now
        ...
        // Tokens exhausted and not yet reset
        const msUntilReset = (FREE_TOKEN_RESET_HOURS * 60 * 60 * 1000) - (now - lastReset);
        showTokenExhaustedModal(msUntilReset);
        return;
    } else {
        tokensRemaining = session.tokens;
    }
...
function updateTokenDisplay() {
    ...
    const pct = Math.round(((FREE_TOKEN_MAX - tokensRemaining) / FREE_TOKEN_MAX) * 100);
    timerDisplay.textContent = `${tokensRemaining} tokens`;

    const timerContent = timerWarning.querySelector('.timer-content');
    if (timerContent) {
        timerContent.innerHTML = `
            <i class="fas fa-coins"></i>
            <span id="timerDisplay">${tokensRemaining} free token${tokensRemaining !== 1 ? 's' : ''} left</span>
            <button onclick="openSubscriptionModal()" class="timer-upgrade-btn">Upgrade <i class="fas fa-crown"></i></button>
        `;
    }
}

async function useToken(action = 'action') {
    if (isPremiumUser) return true;

    if (tokensRemaining <= 0) {
        const userDoc = await getDoc(doc(db, 'users', currentUser.id));
        const session = userDoc.data().freeSession || {};
        const now = Date.now();
        const msUntilReset = (FREE_TOKEN_RESET_HOURS * 60 * 60 * 1000) - (now - (session.lastReset || 0));
        showTokenExhaustedModal(Math.max(msUntilReset, 0));
        return false;
    }
    ...
    await updateDoc(doc(db, 'users', currentUser.id), {
        'freeSession.tokens': tokensRemaining
    });

    updateTokenDisplay();

    // Warn when 5 remaining (95% used)
    if (tokensRemaining === 5 && !tokenWarningShown) {
        ...
        if (timerContent) {
            timerContent.innerHTML = `
                ...
                <span>⚠️ You have used 95% of your free tokens — 5 remaining!</span>
                <button onclick="openSubscriptionModal()" class="timer-upgrade-btn">Upgrade <i class="fas fa-crown"></i></button>
            `;
        }
        showToast("⚠️ You have used 95% of your free tokens — only 5 left! Upgrade for unlimited.", "warning");
    }
    ...
    if (tokensRemaining <= 0) {
        ...
        const now = Date.now();
        const msUntilReset = (FREE_TOKEN_RESET_HOURS * 60 * 60 * 1000) - (now - (session.lastReset || 0));
        setTimeout(() => showTokenExhaustedModal(Math.max(msUntilReset, 0)), 400);
    }
...
async function showTokenExhaustedModal(msUntilReset) {
            ...
            <div style="padding: 40px 32px 32px;">
                <div style="text-align: center; margin-bottom: 24px;">
                    ...
                    <h2 style="font-size: 1.8rem; font-weight: 800; margin-bottom: 8px;">
                        Your Free Tokens <span style="color: #ff4d6d;">Are Used Up</span>
                    </h2>
                    <p style="color: #666; font-size: 0.95rem;">
                        Resets in <strong id="tokenCountdown" style="color:#ff4d6d;">${formatCountdown(msUntilReset)}</strong> — or upgrade for unlimited access ✨
                    </p>
                </div>
                ...
                <p style="
                    ...
                    margin-top: 20px;
                ">
                    <i class="fas fa-shield-alt"></i> Tokens reset in ${formatCountdown(msUntilReset)} • Upgrade anytime • Cancel easily
                </p>
            </div>
        </div>
    `;
    ...
    const resetCountdownInterval = setInterval(() => {
        ...
        if (msLeft <= 0) {
            ...
            if (countdownEl) countdownEl.textContent = formatCountdown(msLeft);
        }
    }, 1000);
    modal.resetCountdownInterval = resetCountdownInterval;
    ...
    window.closeEngagingModal = function() {
        ...
        clearInterval(resetCountdownInterval);
        modal.remove();
        showToast("✨ Your tokens will reset soon! Upgrade anytime to connect with them.", "info");
        disableAllInteractions();
    };
}

// Legacy stub — token system now handles exhaustion via showTokenExhaustedModal
function logoutFreeUser() {
    showTokenExhaustedModal(FREE_TOKEN_RESET_HOURS * 60 * 60 * 1000);
}

// Old showEngagingUpgradeModal replaced by showTokenExhaustedModal above
async function _unusedShowEngagingUpgradeModal() {
    if (document.getElementById('engagingUpgradeModal')) return;
